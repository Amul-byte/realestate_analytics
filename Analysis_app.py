...
# Page config
# ---------------------------
st.set_page_config(page_title="Analysis", page_icon="🏗️", layout="wide")
st.title("🏙️ Analysis App")

df = pd.read_csv('datasets/Data Visualization 1.csv')
new_df = df
...
plt.hist(new_df[new_df['property_type']=='house']['price'], bins=60, alpha=0.5, density=True, label='house')
plt.hist(new_df[new_df['property_type']=='flat']['price'],  bins=60, alpha=0.5, density=True, label='flat')
sns.kdeplot(new_df[new_df['property_type']=='house']['price'], linewidth=2, label='house KDE')
sns.kdeplot(new_df[new_df['property_type']=='flat']['price'],  linewidth=2, label='flat KDE')
plt.legend()
st.pyplot(fig3)
